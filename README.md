...
> **Windows / macOS** 轻量托盘 / 菜单栏工具 — 让 **Codex Desktop** 一键切换到 DeepSeek、通义千问、智谱、Kimi、MiniMax 等国产大模型。
...
**想用 Computer Use（让 Codex 控制桌面 / 浏览器）？** 先打开一次 Codex Desktop，然后：

1. 右键托盘 → **修复 Computer Use（桌面控制）**
2. 按弹窗提示重新打开 Codex，新开对话，输入 `$computer-use`

> 插件页里点「安装」可能一直失败，**不用管**，走托盘修复即可。
...
| Computer Use 插件安装失败 | 托盘 → **修复 Computer Use（桌面控制）**，勿在插件页反复点安装 |
...
- **发版**：见 [RELEASE.md](RELEASE.md)；Windows 一键 `scripts\release.bat` 或 `scripts\release-windows.ps1 -Retag`（同版本热修复）
...
| 修复 Computer Use | `cargo run -- repair-computer-use` |
...
1. **先打开过一次** Codex Desktop
2. 右键托盘 → **修复 Computer Use（桌面控制）**（会自动关闭 Codex 并完成修复）
3. 按弹窗提示：**重新打开 Codex** → **新开对话** → 输入 `$computer-use`

命令行用户也可执行 `codex-helper repair-computer-use`。验证：`codex-helper doctor` 应显示 `computer-use@computer-use-local` 已安装。

**Q：托盘「修复 Computer Use」提示找不到 Codex CLI？**
A：修复功能需要 **Codex Desktop 自带的 CLI**，不是 Helper 自己带的。请确认：

1. 已安装 **Codex Desktop**（仅装 Helper 不够）
2. **完整打开并退出过一次** Codex Desktop（让 Desktop 写入 `~/.codex/config.toml` 和 `node_repl` 路径）
3. 再点托盘 → 修复 Computer Use

若已打开过 Desktop 仍失败，运行 `codex-helper doctor` 查看是否已定位 CLI；微软商店版需使用最新 Helper（会自动搜索 WindowsApps 路径）。
...
1. 已运行 `codex-helper repair-computer-use`（或插件页已成功安装 Computer Use）
